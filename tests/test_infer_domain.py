...
class TestInferDomainTelecom:
    ...
    def test_billing_keyword(self):
        assert _infer_domain("Build a billing support agent") == "Billing Support"

    def test_phone_company(self):
        assert _infer_domain("Verizon-like phone company agent") == "Billing Support"

    def test_telecom(self):
        assert _infer_domain("Telecom customer support bot") == "Billing Support"

    def test_monthly_bill(self):
        assert _infer_domain("Help customers understand their monthly bill") == "Billing Support"

    def test_charges(self):
        assert _infer_domain("Explain charges on a wireless bill") == "Billing Support"

    def test_wireless(self):
        assert _infer_domain("Wireless plan comparison agent") == "Billing Support"

    def test_mobile_plan(self):
        assert _infer_domain("Help with mobile plan selection") == "Billing Support"
